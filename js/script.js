...
async function getsong(floder) {
    ...
    //console.log(floder)
    currentfold = floder
    let a = await fetch(`http://127.0.0.1:5500/${currentfold}/`)
...
async function dispalyalbum(username, folders) {
    ...
    for (let nf of folders) {
        let a = await fetch(`http://127.0.0.1:5500/songs${username}/${nf}/info.json`)
...
async function getfold(username) {
    let f = ""
    f = await fetch(`http://127.0.0.1:5500/songs${username}/`)
    let response
...
async function main() {
    ...
    //     var password=document.getElementById("pass").value;
    //     if(username=="vishnu"&& password=="vishnu0022"){
    //         document.querySelector(".toggleform").style.left="10px"
    //     }
    //     else{
    //         console.log("error")
